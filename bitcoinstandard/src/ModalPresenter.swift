...
class ModalPresenter : Subscriber, Trackable {
    ...
    private func presentBuyController(_ mountPoint: String) {
        ...
        #if Debug || Testflight
            vc = BRWebViewController(bundleName: "bitcoinstandard-frontend-staging", mountPoint: mountPoint, walletManager: walletManager, store: store)
        #else
            vc = BRWebViewController(bundleName: "bitcoinstandard-frontend", mountPoint: mountPoint, walletManager: walletManager, store: store)
        #endif
        vc.startServer()
        vc.preload()
